-- Checking crime_scene from description
SELECT description
...
-- Two incidents happened that day.

-- witnesses could be the accomplice. So finding the names of the witnesses from the interviews table. Also, checking their interviews' transcripts.
...
-- Findling the names of the 3 witnesses from the list of names of people who gave interviews on July 28, 2021. Crime report said that the witnesses mentioned "bakery" in their transcripts.
SELECT name, transcript
FROM interviews
WHERE year = 2021
AND month = 7
...
-- Raymond gave clues. When he leaves the bakery, they called a person and talked for less than a minute. They asked the person on the other end of the call to buy a flight ticket of the earliest flight on July 29, 2021.
...
-- Ruth gave clues- The thief drove away in a car from the bakery, within 10 minutes from the theft. checking the license plates of cars within that time frame. Then, checking out the names of those cars' owners. They could be suspects.
...
-- He escaped to the New York City, as he took the New York City flight.
